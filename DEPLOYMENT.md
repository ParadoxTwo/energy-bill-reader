...
3. **CRITICAL**: Set **"Root Directory"** to: `backend`
   - This is ESSENTIAL to prevent Railway from detecting Node.js and causing build conflicts
   - Railway will only see Python files in the backend directory
   - This avoids the Python environment conflict we've been experiencing
...
- **CRITICAL**: Make sure `Root Directory` is set to **`backend`** in Railway settings (NOT empty)
  - This is the most important setting - it prevents Railway from detecting Node.js
  - Without this, you'll get Python environment conflicts during build
